...
public class Controller extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        } else if (request.getParameter("billInfo") != null) {
            address = methodBillInfo();
        } else if (request.getParameter("orderHistory") != null) {
            address = methodOrderHistory();
        } else {
  ...
  public String methodAddCart(String itemID) {
      // add an row to the shopping cart table
      // add a item with a itemID && userName && make sure that if it
    ...
    if (dbObj != null) {
        ...
        if (item.getLeftInStock() <= 0) {
            request.setAttribute("stock", "Out of Stock!");
        } else {
            int left = item.getLeftInStock() - 1;
            item.setLeftInStock(left);
  ...
  public String methodEmptyCart() {
      // remove all items in shopping cart where this username is the username
      ...
      if (dbObj != null) {
          for (Object b : dbObj) {
              ...
              int left = catalogItem.getLeftInStock() + 1;
              catalogItem.setLeftInStock(left);
  ...
  public String methodViewItem(String itemID) {
      // Go find the item and set the parameters to show 
      // the appropriate description
     Object dbObj = HibernateHelper.getFirstMatch(CatalogItem.class, "itemId", itemID);
      if (dbObj != null) {
        item = (CatalogItem)dbObj;
        if(item.getLeftInStock() < 0) {
            item.setLeftInStock(0);
            HibernateHelper.updateDB(item);
        }
      }
  ...
  public String methodViewCart() {
      // Go get all the rows in shopping car where this username is the username
    List dbObj = HibernateHelper.getListData(ShoppingCart.class, "userName", request.getRemoteUser());
  ...
  public String methodProcessCart() {
    // Go get all the rows in shopping car where this username is the username
    // Calculate the total price of all the items + how many items there are
    List dbObj = HibernateHelper.getListData(ShoppingCart.class, "userName", request.getRemoteUser());
    List cart = new LinkedList();
    int countTotal = 0;
  ...
  public String methodOrderHistory() {
    ...
    return "/secureUser/orderHistory.jsp";
}
  
  public String methodBillInfo() {
    return "/secureUser/billingShippingInformation.jsp";
}
